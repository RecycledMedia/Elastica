...
class Client {
	...
	protected $retryOnException = true;
	 ...
	 * Requires elasticsearch >= 0.19.0
	 *
	 * @param int $id
	 *        	document id
	 * @param array|\Elastica\Script|\Elastica\Document $data
	...
	public function updateDocument($id, $data, $index, $type, array $options = array()) {
		...
		if ($data instanceof Script) {
			...
			$docOptions = $data->getOptions ( array (
					'version',
					'version_type',
					'routing',
					'percolate',
			...
			if ($data instanceof Document && ($data->isAutoPopulate () || $this->getConfigValue ( array (
					'document',
					'autoPopulate' 
			), false )) && ! isset ( $options ['fields'] )) {
		...
		if ($response->isOk () && $data instanceof Document && ($data->isAutoPopulate () || $this->getConfigValue ( array (
				'document',
				'autoPopulate' 
		), false ))) {
